...
public class HierarchyService {
    ...
    public Optional<EmployeeDto> supervisors(final String employeeName) {
        return hierarchy.findEmployee(employeeName).map(e -> Optional.ofNullable(e.getSupervisor())
                        .orElse(EmployeeDto.builder().build())).map(e -> Optional.ofNullable(e.getSupervisor()).map(v -> {
